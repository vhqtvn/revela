module 0x1::simple_map {
    ...
    public fun destroy_empty<T0: store, T1: store>(arg0: SimpleMap<T0, T1>) {
        let SimpleMap<T0, T1> { data: v0 } = arg0;
    ...
    public fun to_vec_pair<T0: store, T1: store>(arg0: SimpleMap<T0, T1>) : (vector<T0>, vector<T1>) {
        ...
        let SimpleMap<T0, T1> { data: v2 } = arg0;
        0x1::vector::reverse<Element<T0, T1>>(&mut v2);
        let v3 = 0x1::vector::length<Element<T0, T1>>(&v2);
        while (v3 > 0) {
            let Element<T0, T1> {
                key   : v4,
                value : v5,
            } = 0x1::vector::pop_back<Element<T0, T1>>(&mut v2);
            0x1::vector::push_back<T0>(&mut v0, v4);
            0x1::vector::push_back<T1>(&mut v1, v5);
            v3 = v3 - 1;
        };
        0x1::vector::destroy_empty<Element<T0, T1>>(v2);
        (v0, v1)
    }
