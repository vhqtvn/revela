module 0x1::stake {
    ...
    public fun next_validator_consensus_infos() : vector<0x1::validator_consensus_info::ValidatorConsensusInfo> acquires StakePool, ValidatorConfig, ValidatorFees, ValidatorPerformance, ValidatorSet {
        let v0 = borrow_global<ValidatorSet>(@0x1);
        let v1 = 0x1::staking_config::get();
        let v2 = borrow_global<ValidatorPerformance>(@0x1);
        let (v3, _) = 0x1::staking_config::get_required_stake(&v1);
        let (v5, v6) = 0x1::staking_config::get_reward_rate(&v1);
        let v7 = 0x1::vector::empty<ValidatorInfo>();
        let v8 = 0;
        let v9 = 0;
        let v10 = 0;
        let v11 = 0x1::vector::length<ValidatorInfo>(&v0.active_validators);
        while (v9 < v11 + 0x1::vector::length<ValidatorInfo>(&v0.pending_active)) {
            let v12 = if (v9 < v11) {
                0x1::vector::borrow<ValidatorInfo>(&v0.active_validators, v9)
            } else {
                0x1::vector::borrow<ValidatorInfo>(&v0.pending_active, v9 - v11)
            };
            let v13 = borrow_global<StakePool>(v12.addr);
            let v14 = 0x1::coin::value<0x1::aptos_coin::AptosCoin>(&v13.active);
            let v15 = if (v9 < v11 && v14 > 0) {
                let v16 = 0x1::vector::borrow<IndividualValidatorPerformance>(&v2.validators, v12.config.validator_index);
                calculate_rewards_amount(v14, v16.successful_proposals, v16.successful_proposals + v16.failed_proposals, v5, v6)
            } else {
                0
            };
            let v17 = 0;
            ...
            if (v20 >= v3) {
                ...
                v10 = v10 + (v20 as u128);
                0x1::vector::push_back<ValidatorInfo>(&mut v7, v22);
                v8 = v8 + 1;
            };
            v9 = v9 + 1;
        };
        ...
        let v26 = ValidatorSet{
            ...
            active_validators   : v7,
            pending_inactive    : v24,
            pending_active      : v25,
            total_voting_power  : v10,
            total_joining_power : 0,
        };
    ...
    public(friend) fun on_new_epoch() acquires AptosCoinCapabilities, StakePool, ValidatorConfig, ValidatorFees, ValidatorPerformance, ValidatorSet {
        ...
        v4 = 0;
        while (v4 < 0x1::vector::length<ValidatorInfo>(v5)) {
            update_stake_pool(v2, 0x1::vector::borrow<ValidatorInfo>(v5, v4).addr, &v1);
            v4 = v4 + 1;
        };
        let v6 = &mut v0.active_validators;
        let v7 = &mut v0.pending_active;
        append<ValidatorInfo>(v6, v7);
        v0.pending_inactive = 0x1::vector::empty<ValidatorInfo>();
        let v8 = 0x1::vector::empty<ValidatorInfo>();
        let (v9, _) = 0x1::staking_config::get_required_stake(&v1);
        let v11 = 0;
        let v12 = 0;
        while (v12 < 0x1::vector::length<ValidatorInfo>(&v0.active_validators)) {
            let v13 = 0x1::vector::borrow_mut<ValidatorInfo>(&mut v0.active_validators, v12).addr;
            let v14 = *borrow_global_mut<ValidatorConfig>(v13);
            let v15 = generate_validator_info(v13, borrow_global_mut<StakePool>(v13), v14);
            if (v15.voting_power >= v9) {
                v11 = v11 + (v15.voting_power as u128);
                0x1::vector::push_back<ValidatorInfo>(&mut v8, v15);
            };
            v12 = v12 + 1;
        };
        v0.active_validators = v8;
        v0.total_voting_power = v11;
        v0.total_joining_power = 0;
        v2.validators = 0x1::vector::empty<IndividualValidatorPerformance>();
        let v16 = 0;
        while (v16 < 0x1::vector::length<ValidatorInfo>(&v0.active_validators)) {
            let v17 = 0x1::vector::borrow_mut<ValidatorInfo>(&mut v0.active_validators, v16);
            v17.config.validator_index = v16;
            borrow_global_mut<ValidatorConfig>(v17.addr).validator_index = v16;
            let v18 = IndividualValidatorPerformance{
                successful_proposals : 0,
                failed_proposals     : 0,
            };
            0x1::vector::push_back<IndividualValidatorPerformance>(&mut v2.validators, v18);
            let v19 = borrow_global_mut<StakePool>(v17.addr);
            let v20 = 0x1::timestamp::now_seconds();
            let v21 = if (0x1::chain_status::is_operating()) {
                get_reconfig_start_time_secs()
            } else {
                v20
            };
            if (v19.locked_until_secs <= v21) {
                v19.locked_until_secs = v20 + 0x1::staking_config::get_recurring_lockup_duration(&v1);
            };
            v16 = v16 + 1;
        };
    ...
    public fun remove_validators(arg0: &signer, arg1: &vector<address>) acquires ValidatorSet {
        ...
        let v2 = &mut v0.pending_inactive;
        let v3 = 0;
        while (v3 < 0x1::vector::length<address>(arg1)) {
            let v4 = find_validator(v1, *0x1::vector::borrow<address>(arg1, v3));
            if (0x1::option::is_some<u64>(&v4)) {
                let v5 = 0x1::vector::swap_remove<ValidatorInfo>(v1, *0x1::option::borrow<u64>(&v4));
                0x1::vector::push_back<ValidatorInfo>(v2, v5);
            };
            v3 = v3 + 1;
        };
    }
    ...
    fun update_stake_pool(arg0: &ValidatorPerformance, arg1: address, arg2: &0x1::staking_config::StakingConfig) acquires AptosCoinCapabilities, StakePool, ValidatorConfig, ValidatorFees {
        ...
        let v7 = &mut v0.active;
        let v8 = distribute_rewards(v7, v3, v4, v5, v6);
        let v9 = &mut v0.pending_inactive;
        let v10 = distribute_rewards(v9, v3, v4, v5, v6);
        let v11 = v8 + v10;
        let v12 = 0x1::coin::extract_all<0x1::aptos_coin::AptosCoin>(&mut v0.pending_active);
        0x1::coin::merge<0x1::aptos_coin::AptosCoin>(&mut v0.active, v12);
        if (0x1::features::collect_and_distribute_gas_fees()) {
            let v13 = &mut borrow_global_mut<ValidatorFees>(@0x1).fees_table;
            if (0x1::table::contains<address, 0x1::coin::Coin<0x1::aptos_coin::AptosCoin>>(v13, arg1)) {
                let v14 = 0x1::table::remove<address, 0x1::coin::Coin<0x1::aptos_coin::AptosCoin>>(v13, arg1);
                0x1::coin::merge<0x1::aptos_coin::AptosCoin>(&mut v0.active, v14);
            };
        };
        if (get_reconfig_start_time_secs() >= v0.locked_until_secs) {
            let v15 = 0x1::coin::extract_all<0x1::aptos_coin::AptosCoin>(&mut v0.pending_inactive);
            0x1::coin::merge<0x1::aptos_coin::AptosCoin>(&mut v0.inactive, v15);
        };
        if (0x1::features::module_event_migration_enabled()) {
            let v16 = DistributeRewards{
                pool_address   : arg1,
                rewards_amount : v11,
            };
            0x1::event::emit<DistributeRewards>(v16);
        };
        let v17 = DistributeRewardsEvent{
            pool_address   : arg1,
            rewards_amount : v11,
        };
        0x1::event::emit_event<DistributeRewardsEvent>(&mut v0.distribute_rewards_events, v17);
    }
