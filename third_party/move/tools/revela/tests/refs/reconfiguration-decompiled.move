module 0x1::reconfiguration {
    ...
    public(friend) fun reconfigure() acquires Configuration {
        let v0 = 0x1::chain_status::is_genesis() || 0x1::timestamp::now_microseconds() == 0;
        if (v0) {
            v0 = true;
        } else {
            v0 = !reconfiguration_enabled();
        };
        if (v0) {
            return
        };
        let v1 = borrow_global_mut<Configuration>(@0x1);
        let v2 = 0x1::timestamp::now_microseconds();
        if (v2 == v1.last_reconfiguration_time) {
            return
        };
        ...
        assert!(v2 > v1.last_reconfiguration_time, 0x1::error::invalid_state(4));
        v1.last_reconfiguration_time = v2;
        v1.epoch = v1.epoch + 1;
        if (0x1::features::module_event_migration_enabled()) {
            let v3 = NewEpoch{epoch: v1.epoch};
            0x1::event::emit<NewEpoch>(v3);
        };
        let v4 = NewEpochEvent{epoch: v1.epoch};
        0x1::event::emit_event<NewEpochEvent>(&mut v1.events, v4);
