module 0x1::capability {
    ...
    public fun delegate<T0>(arg0: Cap<T0>, arg1: &T0, arg2: &signer) acquires CapState {
        ...
        let v2 = &mut borrow_global_mut<CapState<T0>>(arg0.root).delegates;
        add_element<address>(v2, v0);
    }
    ...
    public fun revoke<T0>(arg0: Cap<T0>, arg1: &T0, arg2: address) acquires CapDelegateState, CapState {
        if (exists<CapDelegateState<T0>>(arg2)) {
            let CapDelegateState<T0> {  } = move_from<CapDelegateState<T0>>(arg2);
            let v0 = &mut borrow_global_mut<CapState<T0>>(arg0.root).delegates;
            remove_element<address>(v0, &arg2);
            return
        };
    }
