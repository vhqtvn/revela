    ...
    fn run_basicblock(
        ...
        for inst in basic_block.inner().content.code.iter() {
            ...
            match &inst.bytecode {
                ...
                StacklessBytecode::Branch(_, _, _, src) | StacklessBytecode::Abort(_, src) => {
                    state.var_read(&vec![*src]);
                }

                StacklessBytecode::SpecBlock(_, _)
                | StacklessBytecode::SaveMem(_, _, _)
                | StacklessBytecode::SaveSpecVar(_, _, _)
                | StacklessBytecode::Prop(_, _, _)
